...
AGP_file <- paste(path, "AGP.rds", sep = "")
...
AGP.items <- AGP.cc %>%
  select(ID,
        agp.public_transport_phobia,
        agp.open_spaces_phobia,
        agp.enclosed_spaces_phobia,
        agp.queue_or_crowd_phobia,
        agp.outside_home_alone_phobia,
        agp.phobia_frequency,
        agp.avoid_phobia,
        agp.endure_phobia_with_anxiety,
        agp.require_companion,
        agp.afraid_faint,
        agp.afraid_escape_difficult,
        agp.afraid_help_not_available,
        agp.phobia_lasted,
        agp.phobia_interfered,
        agp.phobia_out_of_proportion,
...
names(AGP.items)<-gsub("AGP.cc.", "", names(AGP.items))
```
...
AGP.items <- AGP.items %>% 
  mutate(agp.screener_numeric = 
           case_when(
             ...
             agp.feared_situations >=2 & (agp.phobia_frequency == "Never" | agp.phobia_frequency == "Only one or two times ever" | agp.phobia_frequency == "Some of the time") ~ 0,
             agp.feared_situations >=2 & (agp.phobia_frequency == "Almost always" | agp.phobia_frequency == "Always") ~ 1
             )
         )
...
AGP.items <- AGP.items %>% 
  mutate(agp.criterion_B_numeric =
           case_when(
             agp.afraid_faint == "No" & agp.afraid_escape_difficult == "No" & agp.afraid_help_not_available == "No" ~ 0,
             agp.afraid_faint == "Yes" ~ 1,
             agp.afraid_escape_difficult == "Yes" ~ 1,
             agp.afraid_help_not_available == "Yes" ~ 1
           ))
...
AGP.items <- AGP.items %>% 
  mutate(agp.criterion_D_numeric =
           case_when(
             agp.avoid_phobia == "No" & agp.endure_phobia_with_anxiety == "No" & agp.require_companion == "No" ~ 0,
             agp.avoid_phobia == "Yes" ~ 1,
             agp.endure_phobia_with_anxiety == "Yes" ~ 1,
             agp.require_companion == "Yes" ~ 1
           ))
  ...
  mutate(agp.diagnosis_numeric = 
           case_when(
             ...
             agp.screener_numeric == 0 | 
               ...
               agp.phobia_out_of_proportion == "No" |
               agp.phobia_lasted_numeric <= 1 |
               (agp.phobia_interfered == "Not at all" | agp.phobia_interfered == "A little")
             ~ 0,
...
saveRDS(object = AGP.clean, file = paste0(export_path, "AGP_cleaned.rds"))

# Saves the R data including the recoding
#save.image(file = "Datasets/Relabelled_cleaned/GLAD_AGP_clean.RData")
```