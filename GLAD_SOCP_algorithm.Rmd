...
SOCP_file <- paste(path, "SOCP.rds", sep = "")
...
SOCP.items <- SOCP.items %>% 
  mutate(socp.criterion_A.numeric =
           case_when(
...
summary(as.factor(SOCP.items$socp.criterion_A.numeric))
```

```{r social phobia screener variable}
#Questionnaire screening variable
SOCP.items <- SOCP.items %>% 
  mutate(socp.screener.numeric = 
           case_when(
             socp.criterion_A.numeric == 0 ~ 0,
             ...
             socp.criterion_A.numeric == 1 & (socp.anx_social_situation_frequency == "Almost always" | socp.anx_social_situation_frequency == "Always") ~ 1,
             is.na(socp.criterion_A.numeric) | is.na(socp.anx_social_situation_frequency) ~ NA_real_
           ))

summary(as.factor(SOCP.items$socp.screener.numeric))
```
...
SOCP.items$socp.diagnosis.numeric <-
  with(SOCP.items,
  dplyr::case_when(
## No social phobia
    #Criterion A. Marked fear in social situations
    (socp.criterion_A.numeric == 0) |
...
freq(SOCP.items$socp.diagnosis.numeric)
...
SOCP.items$socp.social_phobia_symptoms.calc.NA<-with(SOCP.items, 
                                    ifelse(socp.screener.numeric %in% 1 & is.na(socp.anx_others_think), 
...
SOCP.items$socp.social_phobia_symptoms.calc.NA<-with(SOCP.items, 
                                    ifelse(socp.screener.numeric %in% 1 & is.na(socp.anx_irrational), 
                                           socp.social_phobia_symptoms.calc.NA + 1, socp.social_phobia_symptoms.calc.NA))
#Criterion F (lasted 6 months or more) is NA
SOCP.items$socp.social_phobia_symptoms.calc.NA<-with(SOCP.items,
                                    ifelse(socp.screener.numeric %in% 1 & is.na(socp.longest_episode),
                                           socp.social_phobia_symptoms.calc.NA + 1, socp.social_phobia_symptoms.calc.NA))
#Criterion G (impairment) is NA
SOCP.items$socp.social_phobia_symptoms.calc.NA<-with(SOCP.items,
                                    ifelse(socp.screener.numeric %in% 1 & is.na(socp.interference_with_life),
                                           socp.social_phobia_symptoms.calc.NA + 1, socp.social_phobia_symptoms.calc.NA))
...
SOCP.items$socp.diagnosis.numeric <- with(SOCP.items,
                                       ifelse(
                                         is.na(socp.screener.numeric) | 
                                           socp.social_phobia_symptoms.calc.NA > 0,
                                         NA, socp.diagnosis.numeric 
                                       ))
...
summary(SOCP.items$socp.diagnosis.numeric)

SOCP.items$socp.diagnosis <- recode_factor(SOCP.items$socp.diagnosis.numeric,
                             "0" = "No social phobia",
                             "1" = "Social phobia")
...
SOCP.clean <- SOCP.items %>%
  select(ID,
        ...
        socp.longest_episode,
        socp.interference_with_life,
        socp.screener.numeric,
        socp.diagnosis,
        socp.diagnosis.numeric)

#Eport data
saveRDS(object = SOCP.clean, file = paste0(export_path, "SOCP_cleaned.rds"))

# Saves the R data including the recoding
#save.image(file = "../../../Datasets/Relabelled_cleaned/GLAD_SOCP_clean.RData")
```