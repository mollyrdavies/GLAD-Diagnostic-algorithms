...
PAD_file <- paste(path, "PAD.rds", sep = "")
...
PAD.items[PAD.items == "Don't know"] <- NA
PAD.items[PAD.items == "Prefer not to answer"] <- NA
PAD.items[PAD.items == "Seen but not answered"] <- NA
PAD.items[PAD.items == -88] <- NA
PAD.items[PAD.items == -99] <- NA
PAD.items[PAD.items == -77] <- NA
```
...
PAD.items <- PAD.items %>% 
  mutate(pad.diagnosis_numeric =
           case_when(
             ...
             pad.panic_symptoms.NA > 0 | (pad.panic_symptoms_numeric > 3 & no.info > 0) ~ NA_real_
             )
  )
...
saveRDS(object = PAD.clean, file = paste0(export_path, "PAD_cleaned.rds"))

# Saves the R data including the recoding
#save.image(file = "Datasets/Relabelled_cleaned/GLAD_PAD_clean.RData")
```