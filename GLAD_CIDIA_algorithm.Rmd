...
CIDIA.items <- CIDIA.items %>% 
  mutate(cidia.criterion_A_numeric =
           case_when(
             ...
             (cidia.more_than_one_thing == "One thing" & cidia.different_worries == "No") ~ 0,
               #A. occurring  more  days  than not (frequency)
               cidia.most_days == "Yes" &
               #A. about a number of events or activities
               (cidia.more_than_one_thing == "More than one thing" | cidia.different_worries == "Yes") ~ 1
         ))
...
CIDIA.items <- CIDIA.items %>% 
  mutate(cidia.restless_on_edge = 
           recode_factor(cidia.restless_on_edge_numeric,
                         "0" = "No",
                         "1" = "Yes")
         )
```
