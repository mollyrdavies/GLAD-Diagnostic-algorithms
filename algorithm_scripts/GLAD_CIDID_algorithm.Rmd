...
CIDID.items <- CIDID.items %>% 
  mutate(cidid.screener_numeric = 
           case_when(
             cidid.low_mood_numeric == 0 & cidid.anhedonia_numeric == 0 ~ 0,
             cidid.low_mood_numeric == 1 ~ 1,
             cidid.anhedonia_numeric == 1 ~ 1
           )
  )
...
CIDID.items <- CIDID.items %>% 
  mutate(cidid.weight_appetite_change_numeric = 
           case_when(
             cidid.appetite_change_numeric == 0 & cidid.weight_change_numeric == 0 ~ 0,
             cidid.appetite_change_numeric == 1 ~ 1,
             cidid.appetite_change_numeric == 2 ~ 1,
             cidid.appetite_change_numeric == 3 ~ 1,
             cidid.weight_change_numeric == 1 ~ 1,
             cidid.weight_change_numeric == 2 ~ 1,
             cidid.weight_change_numeric == 3 ~ 1
           ))
...
CIDID.items$cidid.criterion_A_score_numeric<-with(CIDID.items, 
                                    ifelse(!is.na(cidid.low_mood_numeric) 
                                           & cidid.low_mood_numeric == 1, 
                                           cidid.criterion_A_score_numeric + 1, cidid.criterion_A_score_numeric))
#Criterion A.2 - anhedonia
CIDID.items$cidid.criterion_A_score_numeric<-with(CIDID.items,
                                    ifelse(!is.na(cidid.anhedonia_numeric) 
                                           & cidid.anhedonia_numeric == 1,
                                           cidid.criterion_A_score_numeric + 1, cidid.criterion_A_score_numeric))
...
CIDID.items$cidid.criterion_A_score_numeric<-with(CIDID.items, 
                                    ifelse(!is.na(cidid.sleep_change_numeric) &
                                             cidid.sleep_change_numeric == 1, 
                                           cidid.criterion_A_score_numeric + 1, cidid.criterion_A_score_numeric))
#Criterion A.6 - fatigue or loss of energy
CIDID.items$cidid.criterion_A_score_numeric<-with(CIDID.items, 
                                    ifelse(!is.na(cidid.fatigue_numeric) & cidid.fatigue_numeric == 1, 
                                           cidid.criterion_A_score_numeric + 1, cidid.criterion_A_score_numeric))
#Criterion A.7 - Feelings of worthlesness
CIDID.items$cidid.criterion_A_score_numeric<-with(CIDID.items, 
                                    ifelse(!is.na(cidid.worthlessness_numeric) 
                                           & cidid.worthlessness_numeric == 1, 
                                           cidid.criterion_A_score_numeric + 1, cidid.criterion_A_score_numeric))
#Criterion A.8 - Diminished ability to concentrate
CIDID.items$cidid.criterion_A_score_numeric<-with(CIDID.items,
                                    ifelse(!is.na(cidid.trouble_concentrating_numeric) 
                                           & cidid.trouble_concentrating_numeric == 1,
                                           cidid.criterion_A_score_numeric + 1, cidid.criterion_A_score_numeric))
#Criterion A.9 - Recurrent thoughts of death or suicidal ideation
CIDID.items$cidid.criterion_A_score_numeric<-with(CIDID.items, 
                                    ifelse(!is.na(cidid.thoughts_of_death_numeric) 
                                           & cidid.thoughts_of_death_numeric == 1, 
                                           cidid.criterion_A_score_numeric + 1, cidid.criterion_A_score_numeric))
...
if (
  sum(ifelse(CIDID.items$cidid.how_often_numeric %in% 1, 1, 0)) > sum(ifelse(CIDID.items$cidid.how_often_numeric %in% 3, 1, 0))
  ) 
    {
      CIDID.items <- CIDID.items %>% 
        mutate(cidid.how_often = 
         case_when(
           cidid.how_often_numeric == 3 ~ 1,
           cidid.how_often_numeric == 2 ~ 2,
           cidid.how_often_numeric == 1 ~ 3
         ))
}
#If "All day long" is less frequently endorsed than "Less than half of the day", cidid.how_long is recoded.
if (
  sum(ifelse(CIDID.items$cidid.how_long_numeric %in% 1, 1, 0)) > sum(ifelse(CIDID.items$cidid.how_long_numeric %in% 4, 1, 0))
  ) {
CIDID.items <- CIDID.items %>% 
  mutate(cidid.how_long_numeric = 
         case_when(
           cidid.how_long_numeric == 1 ~ 4,
           cidid.how_long_numeric == 2 ~ 3,
           cidid.how_long_numeric == 3 ~ 2,
           cidid.how_long_numeric == 4 ~ 1
         ))
}
...
CIDID.items$cidid.criterion_A_score_numeric.NA<-with(CIDID.items, 
                                    ifelse(is.na(cidid.fatigue_numeric), 
                                           cidid.criterion_A_score_numeric.NA + 1, cidid.criterion_A_score_numeric.NA))
...
CIDID.items <- CIDID.items %>% 
  mutate(cidid.diagnosis_numeric =
          ...
          #Ppts are coded 0 if they meet screening criteria but don't meet at least one of the other criteria
               ...
               (cidid.how_long_numeric == 1 | cidid.how_long_numeric == 2) | 
               cidid.how_often_numeric == 1 |
               (cidid.functioning_numeric == 0 | cidid.functioning_numeric == 1)) 
             ~ 0,
          #Ppts are coded as 1 if the meet all criteria
             ...
             cidid.screener_numeric == 1 &
               ...
               cidid.criterion_A_score_numeric >= 5 &
               #A. most of the day
               (cidid.how_long_numeric == 3 | cidid.how_long_numeric == 4) & 
               #A. nearly every day
               (cidid.how_often_numeric == 2 | cidid.how_often_numeric == 3) &
               #B. functional  impairment
               (cidid.functioning_numeric == 2 | cidid.functioning_numeric == 3) 
             ~ 1
           ))
