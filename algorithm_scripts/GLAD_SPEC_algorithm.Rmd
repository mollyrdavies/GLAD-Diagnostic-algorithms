...
SPEC.items <- SPEC.cc %>%
  select(ID,
        ...
        spec.age_first_phobia_numeric,
        spec.age_most_recent_phobia_numeric,
...
SPEC.items$spec.phobias_count_numeric<-with(SPEC.items,
                                                    ifelse(
                                                      spec.environment_phobia %in% "1", 
                                                      spec.phobias_count_numeric + 1, spec.phobias_count_numeric))
#Situation phobia
SPEC.items$spec.phobias_count_numeric<-with(SPEC.items,
                                                    ifelse(
                                                      spec.situation_phobia %in% "1", 
                                                      spec.phobias_count_numeric + 1, spec.phobias_count_numeric))
#Animal phobia
SPEC.items$spec.phobias_count_numeric<-with(SPEC.items,
                                                    ifelse(
                                                      spec.animal_phobia %in% "Yes", 
                                                      spec.phobias_count_numeric + 1, spec.phobias_count_numeric))
#Blood or injection phobia
SPEC.items$spec.phobias_count_numeric<-with(SPEC.items,
                                                    ifelse(
                                                      spec.blood_injection_phobia %in% "Yes", 
                                                      spec.phobias_count_numeric + 1, spec.phobias_count_numeric))
#Other phobia
SPEC.items$spec.phobias_count_numeric<-with(SPEC.items,
                                                    ifelse(
                                                      spec.other_phobia %in% "Yes", 
                                                      spec.phobias_count_numeric + 1, spec.phobias_count_numeric))
...
SPEC.items <- SPEC.items %>% 
  mutate(specific_phobia_screener_numeric = 
           case_when(
             ...
             spec.phobias_count_numeric > 0 & (spec.phobia_frequency == "Never" | spec.phobia_frequency == "Only one or two times ever" | spec.phobia_frequency == "Some of the time") ~ 0,
             spec.phobias_count_numeric > 0 & (spec.phobia_frequency == "Almost always" | spec.phobia_frequency == "Always") ~ 1,
             is.na(spec.environment_phobia) & is.na(spec.situation_phobia) & is.na(spec.animal_phobia) & is.na(spec.blood_injection_phobia) & is.na(spec.other_phobia) ~ NA_real_,
             spec.phobias_count_numeric > 0 & is.na(spec.phobia_frequency) ~ NA_real_
           ))
...
SPEC.items$spec.phobias_NA_count_numeric<-with(SPEC.items,
                                                    ifelse(
                                                      is.na(spec.environment_phobia), 
                                                      spec.phobias_NA_count_numeric + 1, spec.phobias_NA_count_numeric))
#Situation phobia
SPEC.items$spec.phobias_NA_count_numeric<-with(SPEC.items,
                                                    ifelse(
                                                      is.na(spec.situation_phobia), 
                                                      spec.phobias_NA_count_numeric + 1, spec.phobias_NA_count_numeric))
#Animal phobia
SPEC.items$spec.phobias_NA_count_numeric<-with(SPEC.items,
                                                    ifelse(
                                                      is.na(spec.animal_phobia), 
                                                      spec.phobias_NA_count_numeric + 1, spec.phobias_NA_count_numeric))
#Blood or injection phobia
SPEC.items$spec.phobias_NA_count_numeric<-with(SPEC.items,
                                                    ifelse(
                                                      is.na(spec.blood_injection_phobia), 
                                                      spec.phobias_NA_count_numeric + 1, spec.phobias_NA_count_numeric))
#Other phobia
SPEC.items$spec.phobias_NA_count_numeric<-with(SPEC.items,
                                                    ifelse(
                                                      is.na(spec.other_phobia), 
                                                      spec.phobias_NA_count_numeric + 1, spec.phobias_NA_count_numeric))
...
SPEC.items$spec.specific_phobia_screen.calc.NA<-with(SPEC.items,
                                    ifelse(spec.phobias_count_numeric > 0 & is.na(spec.phobia_frequency),
                                           spec.specific_phobia_screen.calc.NA + 1, spec.specific_phobia_screen.calc.NA))
...
SPEC.items$spec.specific_phobia_symptoms.calc.NA<-with(SPEC.items, 
                                    ifelse(specific_phobia_screener_numeric %in% 1 & is.na(spec.phobia_out_of_proportion), 
                                           spec.specific_phobia_symptoms.calc.NA + 1, spec.specific_phobia_symptoms.calc.NA))

#If criterion F (functional impairment) is NA
SPEC.items$spec.specific_phobia_symptoms.calc.NA<-with(SPEC.items,
                                    ifelse(specific_phobia_screener_numeric %in% 1 & is.na(spec.phobia_interfered),
                                           spec.specific_phobia_symptoms.calc.NA + 1, spec.specific_phobia_symptoms.calc.NA))
#If both items for criterion C (avoid or endure) are NA
SPEC.items$spec.specific_phobia_symptoms.calc.NA<-with(SPEC.items, 
                                    ifelse(specific_phobia_screener_numeric %in% 1 & is.na(spec.avoid_phobias) & is.na(spec.endure_phobias_with_anxiety), 
                                           spec.specific_phobia_symptoms.calc.NA + 1, spec.specific_phobia_symptoms.calc.NA))
