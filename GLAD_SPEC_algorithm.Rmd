...
SPEC_file <- paste(path, "SPEC.rds", sep = "")
...
2) If at least one phobia is endorsed, participants must report that frequency of these fears (criterion B) are almost always or always present when exposed to the phobia
...
SPEC.items <- SPEC.items %>% 
  mutate(specific_phobia_screener = 
           case_when(
...
SPEC.items$specific_phobia_screener_noNA <- with(SPEC.items,
                                        ifelse(is.na(specific_phobia_screener), 0, specific_phobia_screener))

summary(as.factor(SPEC.items$specific_phobia_screener))
summary(as.factor(SPEC.items$specific_phobia_screener_noNA))
```
...
SPEC.items <- SPEC.items %>% 
  mutate(specific_phobia_any.numeric <-
           case_when(
#No phobia
        #Criterion A (not met)
...
#Get frequencies
freq(SPEC.items$specific_phobia_any.numeric)
```
...
SPEC.items$spec.specific_phobia_screen.calc.NA<-with(SPEC.items, 
                                    ifelse(
                                      (specific_phobia_any.numeric == 0 | is.na(specific_phobia_any.numeric)) & 
...
SPEC.items$spec.specific_phobia_screen.calc.NA<-with(SPEC.items,
                                    ifelse(specific_phobia_screener_noNA == 1 & is.na(spec.phobia_frequency),
                                           spec.specific_phobia_screen.calc.NA + 1, spec.specific_phobia_screen.calc.NA))
...
Similarly, once participants pass the screeners, if any of the other core symptoms are NA then the participant might have received a diagnosis of specific phobia, but we won't know. If a participant has an NA in this score but was far from meeting diagnostic criteria, then we may still want to keep them as a '0' for diagnosis. This score is therefore counted separately to allow for this flexibility.
...
SPEC.items$spec.specific_phobia_symptoms.calc.NA<-with(SPEC.items, 
                                    ifelse(specific_phobia_screener_noNA == 1 & is.na(spec.phobia_out_of_proportion), 
                                           spec.specific_phobia_symptoms.calc.NA + 1, spec.specific_phobia_symptoms.calc.NA))

#If criterion F (functional impairment) is NA
SPEC.items$spec.specific_phobia_symptoms.calc.NA<-with(SPEC.items,
                                    ifelse(specific_phobia_screener_noNA == 1 & is.na(spec.phobia_interfered),
                                           spec.specific_phobia_symptoms.calc.NA + 1, spec.specific_phobia_symptoms.calc.NA))
#If both items for criterion C (avoid or endure) are NA
SPEC.items$spec.specific_phobia_symptoms.calc.NA<-with(SPEC.items, 
                                    ifelse(specific_phobia_screener_noNA == 1 & is.na(spec.avoid_phobias) & is.na(spec.endure_phobias_with_anxiety), 
                                           spec.specific_phobia_symptoms.calc.NA + 1, spec.specific_phobia_symptoms.calc.NA))
...
SPEC.items$specific_phobia_any.numeric <- with(SPEC.items,
                                       ifelse(
                                         spec.specific_phobia_screen.calc.NA == 1 | 
                                           spec.specific_phobia_symptoms.calc.NA > 0,
                                         NA, specific_phobia_any.numeric
                                       ))
...
SPEC.items$specific_phobia_any.numeric <- with(SPEC.items,
                                    ifelse((!is.na(spec.environment_phobia) & spec.environment_phobia == "No" & 
                                             ...
                                             !is.na(spec.other_phobia) & spec.other_phobia == "No"), 0, specific_phobia_any.numeric))
...
SPEC.items$spec.diagnosis <- recode_factor(SPEC.items$specific_phobia_any.numeric,
                             "0" = "No phobia",
                             "1" = "Any specific phobia"
                             )
#Rename numeric variable
SPEC.items$spec.diagnosis.numeric <- SPEC.items$specific_phobia_any.numeric
```
...
SPEC.items <- SPEC.items %>% 
  mutate(environment_phobia_diagnosis.numeric = 
           case_when(
             spec.environment_phobia == "Yes" & spec.diagnosis.numeric == 1 ~ 0,
             spec.environment_phobia == "No" & spec.diagnosis.numeric == 1 ~ 0,
             spec.diagnosis.numeric == 0 ~ 0,
             is.na(spec.diagnosis.numeric) ~ NA_real_,
             is.na(spec.environment_phobia) ~ NA_real_
           ))
## Recode factor
SPEC.items <- SPEC.items %>% 
  mutate(environment_phobia_diagnosis =
           recode_factor(environment_phobia_diagnosis.numeric,
                         "0" = "No environment phobia",
                         "1" = "Environment phobia"))
#Situational phobia
SPEC.items <- SPEC.items %>% 
  mutate(situation_phobia_diagnosis.numeric = 
           case_when(
             spec.situation_phobia == "Yes" & spec.diagnosis.numeric == 1 ~ 0,
             spec.situation_phobia == "No" & spec.diagnosis.numeric == 1 ~ 0,
             spec.diagnosis.numeric == 0 ~ 0,
             is.na(spec.diagnosis.numeric) ~ NA_real_,
...
## Recode factor
SPEC.items <- SPEC.items %>% 
  mutate(situation_phobia_diagnosis =
           recode_factor(situation_phobia_diagnosis.numeric,
                         "0" = "No situation phobia",
                         "1" = "Situation phobia"))
#Animal phobia
SPEC.items <- SPEC.items %>% 
  mutate(animal_phobia_diagnosis.numeric = 
           case_when(
             spec.animal_phobia == "Yes" & spec.diagnosis.numeric == 1 ~ 0,
             spec.animal_phobia == "No" & spec.diagnosis.numeric == 1 ~ 0,
             spec.diagnosis.numeric == 0 ~ 0,
             is.na(spec.diagnosis.numeric) ~ NA_real_,
...
## Recode factor
SPEC.items <- SPEC.items %>% 
  mutate(animal_phobia_diagnosis =
           recode_factor(animal_phobia_diagnosis.numeric,
                         "0" = "No animal phobia",
                         "1" = "Animal phobia"))
#Blood injection phobia
SPEC.items <- SPEC.items %>% 
  mutate(blood_injection_phobia_diagnosis.numeric = 
           case_when(
             spec.blood_injection_phobia == "Yes" & spec.diagnosis.numeric == 1 ~ 0,
             spec.blood_injection_phobia == "No" & spec.diagnosis.numeric == 1 ~ 0,
             spec.diagnosis.numeric == 0 ~ 0,
             is.na(spec.diagnosis.numeric) ~ NA_real_,
...
## Recode factor
SPEC.items <- SPEC.items %>% 
  mutate(blood_injection_phobia_diagnosis =
           recode_factor(blood_injection_phobia_diagnosis.numeric,
                         "0" = "No blood or injection phobia",
                         "1" = "Blood or injection phobia"))
#Other phobia
SPEC.items <- SPEC.items %>% 
  mutate(other_phobia_diagnosis.numeric = 
           case_when(
             spec.other_phobia == "Yes" & spec.diagnosis.numeric == 1 ~ 0,
             spec.other_phobia == "No" & spec.diagnosis.numeric == 1 ~ 0,
             spec.diagnosis.numeric == 0 ~ 0,
             is.na(spec.diagnosis.numeric) ~ NA_real_,
...
## Recode factor
SPEC.items <- SPEC.items %>% 
  mutate(other_phobia_diagnosis =
           recode_factor(other_phobia_diagnosis.numeric,
                         "0" = "No other phobia",
                         "1" = "Other phobia"))
```
...
SPEC.clean <- SPEC.items %>%
  select(ID,
        ...
        spec.phobia_interfered,
        spec.phobia_out_of_proportion,
        spec.environment_phobia_diagnosis_diagnosis.numeric,
        spec.environment_phobia_diagnosis,
        spec.situation_phobia_diagnosis.numeric,
        spec.situation_phobia_diagnosis,
        spec.animal_phobia_diagnosis.numeric,
        spec.animal_phobia_diagnosis,
        spec.blood_injection_phobia_diagnosis.numeric,
        spec.blood_injection_phobia_diagnosis,
        spec.other_phobia_diagnosis.numeric,
        spec.other_phobia_diagnosis,
        spec.diagnosis,
        spec.diagnosis.numeric)

#Export data
saveRDS(object = SPEC.clean, file = paste0(export_path, "SPEC_cleaned.rds"))

# Save the R data including the recoding
#save.image(file = "../../../Datasets/Relabelled_cleaned/GLAD_SPEC_clean.RData")
```