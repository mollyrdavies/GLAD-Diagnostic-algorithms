...
##The DEM file has more participants overall so NAs will differ, but shouldn't be hugely different
...
sum(ifelse(dem$sample == "NBR" & is.na(dem$highest_education), 1, 0))
sum(ifelse(dat$sample == "NBR" & is.na(dat$highest_education), 1, 0))
sum(ifelse(dem$sample == "GLAD" & is.na(dem$highest_education), 1, 0))
sum(ifelse(dat$sample == "GLAD" & is.na(dat$highest_education), 1, 0))
