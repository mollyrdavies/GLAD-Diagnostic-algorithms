...
glad_dem_renamed <- glad_dem %>% 
  ...
  rename("ID" = "externalDataReference",
         ...
         "age.unc" = "dem.how_old_are_you_now.txt",
...
## Calculating age from birthdates for NBR
...
DEM$age.calc.unc <- ages

#Round age down
DEM$age.calc <- floor(DEM$age.calc.unc)

#Create cleaned age column
#Only use calculated age in cases where self-reported age is unavailable
DEM <- DEM %>% 
  mutate(age.combined.unc = 
           case_when(is.na(age.unc) ~ age.calc,
...
adens <- ggplot(data = DEM.plot, aes(x=age.combined.unc, color="variable")) +
  geom_density() +
...
adens.imp <- ggplot(DEM.plot, aes(x=age.combined.unc, color="variable")) +
  geom_density() +
  scale_color_manual(values = "#E69F00") +
  scale_x_continuous(limits = c(0.00, 105)) +
  geom_vline(xintercept = mean(DEM.plot$age.combined.unc, na.rm = T), 
             color = "blue") +  
  geom_vline(xintercept = mean(DEM.plot$age.combined.unc, na.rm = T) + sd(DEM.plot$age.combined.unc, na.rm = T),
             color = "blue",
             linetype = "dashed") +
  geom_vline(xintercept = mean(DEM.plot$age.combined.unc, na.rm = T) - sd(DEM.plot$age.combined.unc, na.rm = T),
             color = "blue",
...
ggplot(data = subset(DEM.plot, !is.na(age.combined.unc)),
       aes(x = seq_along(age.combined.unc), y = age.combined.unc)) +
  ...
  labs(x = "Index",
       ...
       subtitle = paste("Dashed line represents mean and dotted lines represent ± 1 standard deviation\n",
                        "n = ", sum(!is.na(DEM.plot$age.combined.unc)),
                        "; NA = ", sum(is.na(DEM.plot$age.combined.unc)),
                        "; n(total) = ", length(DEM.plot$age.combined.unc), sep = ""),       
       color = "black") +
  ...
  geom_hline(data = DEM.plot, aes(yintercept = mean(age.combined.unc, na.rm = T)), size = 0.5, linetype = "dashed") +
  geom_hline(data = DEM.plot, aes(yintercept = mean(age.combined.unc, na.rm = T) + sd(age.combined.unc, na.rm = T)), size = 0.5, linetype = "dotted") +
  geom_hline(data = DEM.plot, aes(yintercept = mean(age.combined.unc, na.rm = T) - sd(age.combined.unc, na.rm = T)), size = 0.5, linetype = "dotted")
...
# Set limits for age (add in <16 in case of incorrectly entered ages)
age.m_lower = 16
age.m_upper = 100

# Check how many participants have outlier values in height

age.n_outliers <- which(age.m_lower > DEM$age.combined.unc | DEM$age.combined.unc > age.m_upper)

length(age.n_outliers)

```

```{r remove Age outliers}

if(length(age.n_outliers)>0){
  
# Remove outliers

DEM$age <- with(data = DEM, ifelse(age.m_lower > age.combined.unc | age.combined.unc > age.m_upper, NA, age.combined.unc))
...
ggplot(data = subset(DEM.plot, !is.na(age)),
  ...
  labs(x = "Index",
       ...
       subtitle = paste("Dashed line represents mean and dotted lines represent ± 1 standard deviation\n",
                        ...
                        "; n(outlier) removed = ", length(age.n_outliers), sep = ""),
       color = "black") +
  theme(panel.grid.major.y = element_line(size = 0.5,
                                        linetype = 'dashed',
                                        colour = "gray"),
...
Other professional are disregarded

```{r create highest education variable}
#Create numeric variable
DEM <- DEM %>% 
  mutate(
    highest_education_numeric = 
           case_when(dem.university_numeric == 1 ~ 4,
                     dem.alevels_numeric == 1 ~ 3,
                     dem.NVQ_numeric == 1 ~ 2,
                     dem.gcse_numeric == 1 ~ 1,
                     dem.cse_numeric == 1 ~ 1))

#Create factor variable from numeric version
DEM <- DEM %>% 
  mutate(
    highest_education =
